...
function youtubeEmbed(song){return `<div class="youtube-wrap"><iframe src="https://www.youtube.com/embed/videoseries?list=${PLAYLIST_ID}&index=${song.playlistIndex}&playsinline=1" title="${song.artist} - ${song.title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div>`;}
function songCard(song,side){return `<div class="song-card-wrap">${youtubeEmbed(song)}<button class="song-card" type="button" data-song-id="${song.id}" data-side="${side}"><span class="song-number">PICK ${String(state.matchIndex+1).padStart(2,'0')}</span><span class="song-title">${song.title}</span><span class="song-artist">${song.artist}</span><span class="pick-label">이 곡 선택</span></button></div>`;}
function listCard(song,extra=''){return `<div class="revival-card-wrap">${youtubeEmbed(song)}<button class="revival-card" type="button" data-song-id="${song.id}"><strong>${song.title}</strong><span>${song.artist}${extra}</span><em>이 곡 선택</em></button></div>`;}

function renderFirstRound(){state.stage='first';const a=state.queue[state.matchIndex*2],b=state.queue[state.matchIndex*2+1];stageLabel.textContent='ROUND 01 · 생존전';stageTitle.textContent='더 공연하고 싶은 곡은?';progressText.textContent=`${state.matchIndex+1} / 5`;progressBar.style.width=`${((state.matchIndex+1)/5)*100}%`;hintText.textContent='두 영상을 직접 재생해 들어본 뒤 아래 선택 버튼을 누르세요.';matchArea.className='match-area head-to-head';matchArea.innerHTML=`${songCard(a,'a')}<div class="versus">VS</div>${songCard(b,'b')}`;matchArea.querySelectorAll('.song-card').forEach(btn=>btn.addEventListener('click',()=>chooseFirst(Number(btn.dataset.songId))));}
function chooseFirst(id){const pair=state.queue.slice(state.matchIndex*2,state.matchIndex*2+2);state.winners.push(pair.find(s=>s.id===id));state.losers.push(pair.find(s=>s.id!==id));state.matchIndex++;if(state.matchIndex<5)renderFirstRound();else renderRevival();}
function renderRevival(){state.stage='revival';stageLabel.textContent='ROUND 02 · 패자부활전';stageTitle.textContent='한 곡만 다시 살린다면?';progressText.textContent='1곡 선택';progressBar.style.width='70%';hintText.textContent='탈락한 5곡을 영상으로 다시 들어보고 딱 한 곡을 부활시키세요.';matchArea.className='match-area';matchArea.innerHTML=`<div class="revival-grid">${state.losers.map(song=>listCard(song)).join('')}</div>`;matchArea.querySelectorAll('.revival-card').forEach(btn=>btn.addEventListener('click',()=>chooseRevival(Number(btn.dataset.songId))));}
...
function showResults(){const finalFive=state.finalSix.filter(s=>s.id!==state.eliminatePick.id);state.finalFive=finalFive;gameView.classList.add('hidden');resultView.classList.remove('hidden');$('resultList').innerHTML=finalFive.map((song,i)=>`<li class="result-item"><span class="rank">${i+1}</span><div><strong>${song.title}</strong><span>${song.artist}${song.id===state.revivalPick.id?' · 패자부활 생존':''}</span></div></li>`).join('');window.scrollTo({top:0,behavior:'smooth'});}
