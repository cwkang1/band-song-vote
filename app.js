...
function songCard(song, side) {
  return `<button class="song-card" type="button" data-song-id="${song.id}" data-side="${side}">
...
function renderFirstRound() {
  ...
  hintText.textContent = '카드를 눌러 한 곡을 선택하세요.';
...
function renderRevival() {
  ...
  hintText.textContent = '1차에서 탈락한 5곡 중 딱 한 곡을 부활시키세요.';
  matchArea.className = 'match-area';
  matchArea.innerHTML = `<div class="revival-grid">${state.losers.map(song => `
...
function renderElimination() {
  ...
  hintText.textContent = '이번에는 좋아하는 곡이 아니라, 공연에서 뺄 한 곡을 선택하세요.';
  matchArea.className = 'match-area';
  matchArea.innerHTML = `<div class="revival-grid">${state.finalSix.map(song => `
    <button class="revival-card" type="button" data-song-id="${song.id}">
      <strong>${song.title}</strong><span>${song.artist}${song.id === state.revivalPick.id ? ' · 패자부활' : ''}</span>
    </button>`).join('')}</div>`;
